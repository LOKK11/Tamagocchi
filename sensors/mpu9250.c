...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
	...
	int16_t nx = ((int16_t)rawData[0] << 8) | rawData[1];  // Combine high and low bytes for X-axis acceleration
	int16_t ny = ((int16_t)rawData[2] << 8) | rawData[3];  // Combine high and low bytes for Y-axis acceleration
	int16_t nz = ((int16_t)rawData[4] << 8) | rawData[5];  // Combine high and low bytes for Z-axis acceleration
    int16_t mx = ((int16_t)rawData[8] << 8) | rawData[9];  // Combine high and low bytes for X-axis gyroscope
    int16_t my = ((int16_t)rawData[10] << 8) | rawData[11];  // Combine high and low bytes for Y-axis gyroscope
	int16_t mz = ((int16_t)rawData[12] << 8) | rawData[13]; // Combine high and low bytes for Z-axis gyroscope
