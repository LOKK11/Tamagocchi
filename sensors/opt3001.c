...
double opt3001_get_data(I2C_Handle *i2c) {
    ...
    // JTKJ: Find out the correct buffer sizes (n) with this sensor?
     ...
     I2C_Transaction i2cMessage;
         ...
         txBuffer[0] = OPT3001_REG_RESULT; // Register address in the sensor to read result
         i2cMessage.writeBuf = txBuffer;   // Set the transmission buffer
         i2cMessage.writeCount = 1;        // Send 1 byte
         i2cMessage.readBuf = rxBuffer;    // Set the reception buffer
	...
	if (opt3001_get_status(i2c) & OPT3001_DATA_READY) {
		...
		if (I2C_transfer(*i2c, &i2cMessage)) {
		    ...
		    uint16_t rawLux = (rxBuffer[0] << 8) | rxBuffer[1];
		                lux = 0.01 * (1 << (rawLux >> 12)) * (rawLux & 0xFFF);
